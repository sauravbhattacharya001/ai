...
class ModelCard:
    ...
    def to_html(self) -> str:
        ...
        for r in self.risks:
            ...
            <div class="risk-card">
                <div class="risk-header">
                    ...
                    <span class="badge" style="background:{color}">{r.level.value.upper()}</span>
                </div>
                <p>{r.description}</p>
        ...
        eval_rows = ""
        for e in self.evaluations:
            pct = f"{e.score/e.max_score*100:.0f}%" if e.max_score else "N/A"
            eval_rows += f"<tr><td>{e.benchmark}</td><td>{e.score}/{e.max_score}</td><td>{pct}</td><td>{e.date}</td><td>{e.notes}</td></tr>"

        def _list_html(items: List[str], prefix: str = "") -> str:
            return "".join(f"<li>{prefix}{i}</li>" for i in items)
        ...
        <title>Model Card — {c.model_name}</title>
        <style>
        ...
        <body>
        <div class="container">
          <h1>🛡️ Model Card: {c.model_name}</h1>
          <div class="meta">
            <span>Version {c.model_version}</span>
            <span>Type: {c.model_type}</span>
          ...
          {"<h2>Overview</h2><p>" + c.description + "</p>" if c.description else ""}
          {"<h2>Task</h2><p>" + c.task + "</p>" if c.task else ""}
          ...
          {"<h2>Training Data</h2><p>" + c.training_data_notes + "</p>" if c.training_data_notes else ""}

          <div class="footer">
            {"<p>Contact: " + c.contact + "</p>" if c.contact else ""}
            {"<p>License: " + c.license + "</p>" if c.license else ""}
