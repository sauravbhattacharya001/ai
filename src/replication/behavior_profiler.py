...
class BehaviorProfiler:
    ...
    def _analyze_agent(
        ...
        # Pre-compute category counts and resources once — shared across
        # _check_frequency, _check_entropy, _check_category_ratios, and
        # _check_new_resources to eliminate 4 redundant O(N) passes.
        cat_counts: Counter[ActionCategory] = Counter()
        resources: Set[str] = set()
        for a in actions_sorted:
            cat_counts[a.category] += 1
        ...
        n = len(actions_sorted)
        obs_dist = {cat: count / n for cat, count in cat_counts.items()} if n > 0 else {}

        anomalies: List[Anomaly] = []

        # 1. Category frequency analysis
        anomalies.extend(
            self._check_frequency(agent_id, actions_sorted, baseline, cat_counts)
        )
        ...
        anomalies.extend(
            self._check_new_resources(agent_id, actions_sorted, baseline, resources)
        )
        ...
        # 6. Entropy shift
        anomalies.extend(
            self._check_entropy(agent_id, actions_sorted, baseline, cat_counts, obs_dist)
        )
        ...
        anomalies.extend(
            self._check_category_ratios(agent_id, actions_sorted, baseline, obs_dist)
        )
    ...
    def _check_frequency(
        ...
        cat_counts: Counter[ActionCategory] | None = None,
    ) -> List[Anomaly]:
    ...
    def _check_entropy(
        ...
        baseline: AgentBaseline,
        cat_counts: Counter[ActionCategory] | None = None,
        obs_dist: Dict[ActionCategory, float] | None = None,
    ) -> List[Anomaly]:
        ...
        if cat_counts is None or obs_dist is None:
            cat_counts = Counter(a.category for a in actions)
    ...
    def _check_category_ratios(
        ...
        if obs_dist is None:
            cat_counts: Counter[ActionCategory] = Counter(a.category for a in actions)
            obs_dist = {cat: count / n for cat, count in cat_counts.items()}
